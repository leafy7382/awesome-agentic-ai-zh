...
| 基礎安全與隱私 | 簡單的單一服務部署，適合信任的本地程式。 | 需要獨立的權限管理與沙箱隔離機制。 |

#### 3. 「寫不寫 MCP」的工程決策路徑：以需求與可用度為本，切忌硬性規定

即便是電資學院（CS/EE）背景的學員，也**千萬不要把「自己動手寫 MCP 伺服器」當作硬性規定的作業或必經之路**。軟體工程的的核心原則是**「先找現成工具，再考慮自動生成，最後才手動實作」**。以下是面對 MCP 需求時，最務實的決策路徑：
...
```
                  ...
                  ┌───────────────┴───────────────┐
                  ▼                               ▼
      【現成優先】找現成工具              【沒有現成工具】評估需求
      先查 mcp-skills-catalog          若是單次、專案內使用的工具
      或 awesome-mcp-servers           直接在 Stage 3 寫 Local Tool 即可
                  │                               │
            ┌─────┴─────┐                         ▼
            ▼           ▼                 【需要跨 Host 復用】
         有現成工具   無現成工具 ───┐     決定使用 MCP，但不要自己寫！
            │                       │             │
            ▼                       ▼             ▼
        直接套用安裝          【AI 生成】叫 AI 寫   【極少數例外】手動刻
                            給 AI API 規格與     處理極精密硬體儀器/
                            MCP boilerplate，    機密實驗室設備串接，
                            生成後直接執行       或複雜的安全隔離邊界
```

*   **極簡化原則：非電資與電資通用的 AI 協作心法**：
    *   **非電資學院/非技術背景**：遇到 MCP 需求時，**絕對應該叫 AI 寫**。你只需要把目標 API 的規格書、想要的工具輸入/輸出，丟給 Claude 或其他 LLM，並命令它：「請幫我用 Python MCP SDK 寫一個簡易的 MCP 伺服器，並提供 `mcp.json` 的設定檔範例。」生成後直接透過 `npx` 或 `python` 跑起來即可。
    *   **電資學院/技術背景**：不要浪費生命去手動編寫 JSON-RPC 序列化與通訊協定的樣板程式碼（Boilerplate）。應該著重於**「工具背後的領域邏輯（Domain Logic）」**與**「API 安全設計」**，通訊與協定的膠水程式碼同樣交給 AI 生成，只有當需要**對接實驗室的特殊硬體儀器**、**內部私有資料庫**，或是面臨**嚴格的本地系統隱私與權限邊界（Sandbox）**時，才由工程師進行程式碼微調與安全審查。
    *   **工具可用度優先**：只要市面上（或 awesome 列表）有現成的 Google Drive、Slack、Postgres 等 MCP 伺服器，就直接拿來用。不要重新造輪子。
