...
### 5.2.0 — 传统 Tool Use 到 MCP 的过渡桥梁与决策架构

在进入 MCP 协议前，我们必须先厘清一个关键概念：**既然我们在 Stage 3 已经学过 Tool Use（工具使用），为什么还要大费周章引入 MCP 这种 Client-Server 协议？**

#### 1. 传统 In-Process Tool Use vs. MCP

在传统模式中，工具通常是**进程内（In-Process）**直接执行的函数；而在 MCP 中，工具由一个**独立进程（Subprocess）**以服务器（Server）的形式提供，主机端（Host/Client）通过 JSON-RPC 协议与其通信。

我们用一段并排的代码概念来理解两者的差异：

```python
# ==========================================
# 传统模式 (In-Process)：工具是主程序内的一个函数
# ==========================================
from openai import OpenAI
client = OpenAI()

def my_local_tool(arg):
    return f"Processed {arg}" # 工具直接在同一个进程执行
...
# 取得 tool_calls 后，在本地主程序直接执行 my_local_tool()
```

```python
# ==========================================
# MCP 模式 (Client-Server)：工具是由独立子进程提供的服务
# ==========================================
# 主机端 (Host) 不需要知道工具如何实现，只负责发送 JSON-RPC 请求给 MCP 服务器
# MCP 服务器 (Server) 负责执行工具，并以标准格式返回结果
import asyncio
from mcp import ClientSession, StdioServerParameters
from mcp.client.stdio import stdio_client

async def run_mcp_tool():
    # 建立与独立 MCP 服务器进程的连线 (使用 stdio)
...
引入 MCP 的本质是为了解耦“模型主机端”与“工具提供端”。但这也带来了系统复杂度，以下是决定是否使用 MCP 的决策矩阵：

| 维度 | 选择“不用 MCP”（直接进程内对接） | 选择“使用 MCP” |
| :--- | :--- | :--- |
| **开发场景** | 自建单一用途 Agent，工具只会在此项目中使用。 | 撰写一个工具，希望 Cursor、Claude Code、其他人的 Agent 都能直接套用。 |
| **运行环境** | 工具函数与 LLM 调用代码在同一个进程内执行，共享内存。 | 工具需要独立运行环境（如不同开发语言、Docker 沙箱、外部数据库连接）。 |
| **架构复杂度** | 单一项目或简单目录，易于调试与快速原型开发。 | 模块化微服务架构，工具代码会独立更新与维护。 |
| 基础安全与隐私 | 简单的单一服务部署，适合信任的本地程序。 | 需要独立的权限管理与沙箱隔离机制。 |

#### 3. 「写不写 MCP」的工程决策路径：以需求与可用度为本，切忌硬性规定

即便是电资学院（CS/EE）背景的学员，也**千万不要把「自己动手写 MCP 服务器」当作硬性规定的作业或必经之路**。软件工程的核心原则是**「先找现成工具，再考虑自动生成，最后才手动实现」**。以下是面对 MCP 需求时，最务实的决策路径：
...
```
                  ...
                  ┌───────────────┴───────────────┐
                  ▼                               ▼
      【现成优先】找现成工具              【没有现成工具】评估需求
      先查 mcp-skills-catalog          若是单次、项目内使用的工具
      或 awesome-mcp-servers           直接在 Stage 3 写 Local Tool 即可
                  │                               │
            ┌─────┴─────┐                         ▼
            ▼           ▼                 【需要跨 Host 复用】
         有现成工具   无现成工具 ───┐     决定使用 MCP，但不要自己写！
            │                       │             │
            ▼                       ▼             ▼
        直接套用安装          【AI 生成】叫 AI 写   【极少数例外】手动刻
                            给 AI API 规格与     处理极精密硬件仪器/
                            MCP boilerplate，    机密实验室设备对接，
                            生成后直接运行       或复杂的安全隔离边界
```

*   **极简化原则：非电资与电资通用的 AI 协作心法**：
    *   **非电资学院/非技术背景**：遇到 MCP 需求时，**绝对应该叫 AI 写**。你只需要把目标 API 的规格书、想要的工具输入/输出，丢给 Claude 或其他 LLM，并命令它：「请帮我用 Python MCP SDK 写一个简易的 MCP 服务器，并提供 `mcp.json` 的配置文件示例。」生成后直接通过 `npx` 或 `python` 跑起来即可。
    *   **电资学院/技术背景**：不要浪费生命去手动编写 JSON-RPC 序列化与通信协议的样板代码（Boilerplate）。应该着重于**「工具背后的领域逻辑（Domain Logic）」**与**「API 安全设计」**，通信与协议的胶水代码同样交给 AI 生成，只有当需要**对接实验室的特殊硬件仪器**、**内部私有数据库**，或是面临**严格的本地系统隐私与权限边界（Sandbox）**时，才由工程师进行代码微调与安全审查。
    *   **工具可用度优先**：只要市面上（或 awesome 列表）有现成的 Google Drive、Slack、Postgres 等 MCP 服务器，就直接拿来用。不要重新造轮子。
