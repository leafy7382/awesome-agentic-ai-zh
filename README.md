...
🎓 **大專院校師生與跨學科研究人員專屬 AI Agent 共學指引推薦版本**
...
> 🎓 **大專院校師生專屬分軌導讀指引**：
> 為了滿足高校跨學科（理工資電、生醫光電、管理商學、人文社會等）不同學術背景師生的多元需求，本學習地圖特別設計了兩條獨立且互補的學習路徑，讀者可依據自身背景與研究/教學目的自由選擇：
> 
> * **Track A — CLI Power User（現成工具超能用戶路徑）**：
>   * **適合對象**：非資工資電背景的師生與研究人員（如生醫、管理、人社等），或想要將 AI 快速融入日常研究、教學、寫作與行政流程（例如：自動化文獻整理、研究報告摘要、程式碼輔助生成）的超能使用者。
>   * **學習重點**：不重複造輪子，專注於「如何配置與使用」現成的強大 CLI Agent（如 Claude Code / Aider）。學習編寫 `CLAUDE.md` 規則檔案、串接與設定現成的 MCP 伺服器，用最低的開發成本獲取最高的即時生產力回報。
> 
> * **Track B — Agent Builder（代理人系統建構者路徑）**：
>   * **適合對象**：資工、資電、理工科系師生，有程式開發基礎，或實驗室研究方向涉及自建 Agent 系統、自動化分析平台與客製化企業級 AI 應用的開發者。
>   * **學習重點**：深入技術底層與軟體架構，從零用 Python 實作 ReAct 推理迴圈、設計資料流與記憶體治理架構（如背景記憶壓縮、上下文摺疊、優先權衰減主動修剪與策略驅動的記憶工具）、實作工具的防禦性校驗與隔離沙箱，並深入設計多代理人（Multi-Agent）編排與死鎖熔斷機制。
